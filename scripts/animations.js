...
export async function performNervous(token) {
  ...
  seq.play()
    ...
    seq.effect()
      ...
      .mirrorX(isMirrored)
      .scaleIn(0, 1000, { ease: "easeOutElastic" })
      .attachTo(token, { offset: { x: -0.25 * token.document.width, y: ((0.9 - Math.abs(token.document.texture.scaleX)) / 2) - 0.35 }, gridUnits: true, align: "center", edge: "inner", local: getTokenRotation(token) })
    ...
    seq.effect()
      ...
      .scaleToObject(0.5, { considerTokenScale: true })
      .scaleIn(0, 1000, { ease: "easeOutElastic" })
      .attachTo(token, { offset: { x: 0.2 * token.document.width, y: ((0.9 - Math.abs(token.document.texture.scaleX)) / 2) - 0.35 }, gridUnits: true, align: "center", edge: "inner", local: getTokenRotation(token) })
      .mirrorX(!isMirrored)
      .loopProperty("sprite", "scale.x", { from: 0.5, to: 1, duration: 200, ease: "easeOutQuint" })
      .loopProperty("sprite", "position.x", { from: 0, to: 0.1, gridUnits: true, duration: 200, ease: "easeOutCirc" })
      .loopProperty("sprite", "scale.y", { from: 0.5, to: 1, duration: 200, ease: "easeOutQuint" })
